		...
		public:
			...
			*/
			T& Get() 
				...
				if( p==nullptr )
					{
						throw std::runtime_error("T& ssfAttribute::Get() trying to get a reference to an empty attribute");
					}
				return *p;
				}
			...
			*/
			const T& Get() const
				...
				if( p==nullptr )
					{
						throw std::runtime_error("const T& ssfAttribute::Get() const: trying to get a reference to an empty attribute");
					}
				return *p;
				}
