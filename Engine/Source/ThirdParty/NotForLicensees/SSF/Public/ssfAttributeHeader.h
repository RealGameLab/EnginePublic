...
namespace ssf
	...
	class ssfAttributeHeader 
		...
		public:
			...
			void SetFromDataType( const char *name , double data )
				{
			...
			void SetFromDataType( const char *name , int data )
				{
				this->Name = string(name);
	...
	template<class T> void ssfAttributeHeader::WriteAttributeToOutputStream( ssfBinaryOutputStream *s , const char *name , T *data )
		...
		if( this->Name != dbg_header.Name ||
			...
			{
				throw std::runtime_error("Debug exception: In ssfAttributeHeader::WriteAttributeToOutputStream(): The data does not match the attribute header");
			}
#endif
